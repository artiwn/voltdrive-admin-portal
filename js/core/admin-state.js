(function(){
  ...
  const seed={
    ...
    userDirectory:[
      {id:'USR-001',name:'Ani Grigoryan',email:'ani.g@voltdrive.am',companyId:'CMP-001',roleId:'ROLE-PLATFORM-ADMIN',scope:'Platform · All companies',status:'active',twoFactor:true,lastLogin:'Today · 09:42',source:'Internal',createdAt:'2025-11-03'},
      {id:'USR-002',name:'Arman Hakobyan',email:'arman.h@voltdrive.am',companyId:'CMP-001',roleId:'ROLE-COMPANY-ADMIN',scope:'VoltDrive Armenia · All sites',status:'active',twoFactor:true,lastLogin:'Today · 10:08',source:'Internal',createdAt:'2025-12-09'},
      {id:'USR-003',name:'Mane Sargsyan',email:'mane.s@voltdrive.am',companyId:'CMP-001',roleId:'ROLE-OP-SUPERVISOR',scope:'VoltDrive Armenia · Armenia',status:'active',twoFactor:true,lastLogin:'Today · 08:55',source:'Internal',createdAt:'2026-01-17'},
      {id:'USR-004',name:'Gor Petrosyan',email:'gor.p@voltdrive.am',companyId:'CMP-001',roleId:'ROLE-OPERATOR',scope:'VoltDrive Armenia · Yerevan sites',status:'active',twoFactor:true,lastLogin:'Yesterday · 22:31',source:'Internal',createdAt:'2026-02-21'},
      {id:'USR-005',name:'Narek Avetisyan',email:'narek.a@voltdrive.am',companyId:'CMP-001',roleId:'ROLE-FLEET-MANAGER',scope:'VoltDrive Armenia · Corporate fleet',status:'active',twoFactor:true,lastLogin:'Today · 07:12',source:'Internal',createdAt:'2026-03-04'},
      {id:'USR-006',name:'Tigran Martirosyan',email:'tigran.m@voltdrive.am',companyId:'CMP-001',roleId:'ROLE-TECHNICIAN',scope:'VoltDrive Armenia · Yerevan + Kotayk',status:'active',twoFactor:false,lastLogin:'Yesterday · 18:04',source:'Internal',createdAt:'2026-03-18'},
      {id:'USR-007',name:'Lusine Harutyunyan',email:'lusine.h@voltdrive.am',companyId:'CMP-001',roleId:'ROLE-FINANCE',scope:'VoltDrive Armenia · Finance',status:'active',twoFactor:true,lastLogin:'Today · 09:14',source:'Internal',createdAt:'2026-04-02'},
      {id:'USR-008',name:'Sona Mkrtchyan',email:'sona.m@voltdrive.am',companyId:'CMP-001',roleId:'ROLE-SUPPORT',scope:'VoltDrive Armenia · Customer support',status:'active',twoFactor:true,lastLogin:'Today · 10:26',source:'Internal',createdAt:'2026-04-15'},
      {id:'USR-009',name:'Davit Khachatryan',email:'davit.k@araratmobility.am',companyId:'CMP-002',roleId:'ROLE-COMPANY-ADMIN',scope:'Ararat Mobility · All sites',status:'active',twoFactor:true,lastLogin:'Yesterday · 16:20',source:'Company SSO',createdAt:'2026-05-07'},
      {id:'USR-010',name:'Irina Melikyan',email:'irina.m@sevancharge.am',companyId:'CMP-003',roleId:'ROLE-AUDITOR',scope:'Sevan Charge · Read only',status:'active',twoFactor:true,lastLogin:'Aug 15 · 12:11',source:'Internal',createdAt:'2026-05-19'},
      {id:'USR-011',name:'Karen Grigoryan',email:'karen.g@nreroute.ge',companyId:'CMP-004',roleId:'ROLE-COMPANY-ADMIN',scope:'North Route Energy · Georgia',status:'setup',twoFactor:false,lastLogin:'Never',source:'Invitation',createdAt:'2026-08-12'},
      {id:'USR-012',name:'Vahan Simonyan',email:'vahan.s@voltdrive.am',companyId:'CMP-001',roleId:'ROLE-OPERATOR',scope:'VoltDrive Armenia · Gyumri site',status:'suspended',twoFactor:true,lastLogin:'Aug 09 · 19:42',source:'Internal',createdAt:'2026-02-09'}
    ],
    permissionCatalog:[
    ...
    aiPolicies:{minimumConfidence:80,highImpactApproval:true,allowAutonomousLowRisk:true,allowTariffPublish:false,allowSessionStop:false,allowChargerDisable:false,allowRefund:false,allowMaintenanceReviewCreation:true,modelEvaluationDays:30,maxAutomaticActionsPerHour:50,retainDecisionAuditDays:365,requireAudit:true,explainRecommendations:true,driftAlerting:true},
    reportDefinitions:[
      {id:'RPT-001',name:'Platform Executive Summary',category:'Executive',scope:'Platform · All companies',owner:'Platform Administration',schedule:'Every Monday · 08:00',format:'PDF + CSV',status:'active',lastRun:'2026-08-17 08:00',nextRun:'2026-08-24 08:00',dataDomains:['Companies','Sites','Sessions','Energy','Revenue','Maintenance'],description:'High-level platform administration summary across organizational, operational and commercial domains.'},
      {id:'RPT-002',name:'Charger Reliability & Utilization',category:'Operations',scope:'Platform · Charging network',owner:'Network Analytics',schedule:'Daily · 07:00',format:'CSV',status:'active',lastRun:'2026-08-17 07:00',nextRun:'2026-08-18 07:00',dataDomains:['Charger uptime','Successful sessions','Failed sessions','Energy delivered','Utilization'],description:'Advanced charger reliability and utilization report aligned to platform reporting requirements.'},
      {id:'RPT-003',name:'Financial Performance & Profitability',category:'Finance',scope:'VoltDrive Armenia',owner:'Finance',schedule:'Daily close · 08:30',format:'CSV + XLSX',status:'active',lastRun:'2026-08-17 08:30',nextRun:'2026-08-18 08:30',dataDomains:['Revenue','Electricity cost','Taxes','Commissions','Profitability','Settlements'],description:'Revenue, cost, tax, commission and profitability view by company, site and charger.'},
      {id:'RPT-004',name:'Reservation & Parking Performance',category:'Customer operations',scope:'Public charging · Armenia',owner:'Customer Operations',schedule:'Weekly · Monday',format:'CSV',status:'active',lastRun:'2026-08-17 08:15',nextRun:'2026-08-24 08:15',dataDomains:['Reservations','Cancellations','No-shows','Parking bays','Idle fees'],description:'Reservation, cancellation, no-show and parking-bay utilization reporting.'},
      {id:'RPT-005',name:'Maintenance & SLA Performance',category:'Maintenance',scope:'Managed chargers · Armenia',owner:'Maintenance Engineering',schedule:'Weekly · Monday',format:'PDF + CSV',status:'active',lastRun:'2026-08-17 08:20',nextRun:'2026-08-24 08:20',dataDomains:['Maintenance tickets','Repair time','Repeat failures','SLA','Parts'],description:'Maintenance execution, SLA, repair duration and repeated-failure analysis.'},
      {id:'RPT-006',name:'Fleet & Energy Cost Review',category:'Fleet & Energy',scope:'Corporate fleets · Armenia',owner:'Fleet Operations',schedule:'Monthly',format:'CSV',status:'active',lastRun:'2026-08-01 09:00',nextRun:'2026-09-01 09:00',dataDomains:['Fleet charging cost','Vehicle energy','Depot load','Peak demand','Renewable energy'],description:'Fleet charging cost, depot load and renewable-energy reporting.'},
      {id:'RPT-007',name:'Security & Access Compliance',category:'Security',scope:'Platform',owner:'Platform Security',schedule:'Weekly · Friday',format:'PDF + CSV',status:'active',lastRun:'2026-08-14 17:00',nextRun:'2026-08-21 17:00',dataDomains:['Users','Roles','2FA','Certificates','Blocked identities','Security events'],description:'Administrative access, MFA, certificate lifecycle and security event compliance report.'},
      {id:'RPT-008',name:'Integration & Reconciliation Health',category:'Integrations',scope:'Production integrations',owner:'Platform Integrations',schedule:'Daily · 09:00',format:'CSV',status:'active',lastRun:'2026-08-17 09:00',nextRun:'2026-08-18 09:00',dataDomains:['ERP sync','API delivery','Webhooks','Mappings','Reconciliation'],description:'ERP/API synchronization health, mapping review and financial reconciliation status.'}
    ],
    reportRuns:[
    ...
    audit:[
      {id:'AUD-101',date:'2026-08-17',time:'10:18',icon:'♙',module:'Users & Access',actor:'Ani Grigoryan',severity:'warning',title:'Role approval requested',detail:'Mariam Hovsepyan · Operator Supervisor',source:'Admin Portal'},
      {id:'AUD-102',date:'2026-08-17',time:'09:35',icon:'⇄',module:'ERP & Integrations',actor:'Ani Grigoryan',severity:'info',title:'ERP mapping updated',detail:'Settlement export · 1C ERP Armenia',source:'Admin Portal'},
      {id:'AUD-103',date:'2026-08-17',time:'09:12',icon:'¤',module:'Tariffs & Pricing',actor:'Ani Grigoryan',severity:'info',title:'Tariff published',detail:'Yerevan DC Peak · v3',source:'Admin Portal'},
      {id:'AUD-104',date:'2026-08-17',time:'08:54',icon:'◇',module:'Security & Certificates',actor:'Platform Security',severity:'warning',title:'Certificate inventory checked',detail:'2 certificates require renewal',source:'Security Service'},
      {id:'AUD-105',date:'2026-08-17',time:'08:22',icon:'▣',module:'Companies',actor:'Ani Grigoryan',severity:'info',title:'Company configuration changed',detail:'Ararat Mobility · billing profile',source:'Admin Portal'},
      {id:'AUD-106',date:'2026-08-16',time:'18:42',icon:'▤',module:'Payments',actor:'Lusine Harutyunyan',severity:'info',title:'Payment policy reviewed',detail:'Preauthorization and refund controls · VoltDrive Armenia',source:'Admin Portal'},
      {id:'AUD-107',date:'2026-08-16',time:'17:12',icon:'⇄',module:'ERP & Integrations',actor:'Integration Service',severity:'warning',title:'ERP acknowledgement timeout recovered',detail:'1C ERP Armenia · automatic retry succeeded',source:'Integration Service'},
      {id:'AUD-108',date:'2026-08-16',time:'15:22',icon:'✦',module:'AI & Automation',actor:'AI Governance',severity:'warning',title:'Finance anomaly sent for review',detail:'Settlement variance cluster · no autonomous payment action',source:'AI Governance'},
      {id:'AUD-109',date:'2026-08-15',time:'11:04',icon:'↻',module:'Firmware',actor:'Platform Engineering',severity:'info',title:'Firmware campaign approved',detail:'FW-2026.08 · staged rollout',source:'Admin Portal'},
      {id:'AUD-110',date:'2026-08-14',time:'16:30',icon:'◇',module:'Security & Certificates',actor:'Platform Security',severity:'critical',title:'Compromised RFID blocked',detail:'RFID-88421 · credential deny-list entry',source:'Security Service'}
    ]
  };
  ...
  state.tariffProfiles=(Array.isArray(state.tariffProfiles)?state.tariffProfiles:seed.tariffProfiles).map((profile,index)=>({
    id:profile.id||`TAR-${String(index+1).padStart(3,'0')}`,name:'Untitled tariff',version:1,countryCode:'AM',currency:'AMD',companyId:'CMP-001',scopeType:'Market',scopeValue:'All eligible charging',audience:'Driver',connectorClass:'All connectors',status:'draft',taxProfileId:'TAX-001',energyRate:0,minuteRate:0,connectionFee:0,reservationFee:0,parkingFee:0,idleFee:0,discountType:'None',discountValue:0,scheduleMode:'Always',scheduleDays:'Every day',scheduleStart:'00:00',scheduleEnd:'23:59',effectiveFrom:'2026-08-17',effectiveTo:'',notes:'',lastUpdated:'2026-08-17 11:30',
  ...
  state.firmwareEvents=(Array.isArray(state.firmwareEvents)?state.firmwareEvents:seed.firmwareEvents).map((item,index)=>({id:item.id||`FWE-${String(index+1).padStart(3,'0')}`,campaignId:'',charger:'Unknown charger',model:'Unknown model',fromVersion:'—',toVersion:'—',status:'review',time:'',detail:'',...(firmwareEventSeedById[item.id]||{}),...item}));
  ...
  state.reportRuns=(Array.isArray(state.reportRuns)?state.reportRuns:seed.reportRuns).map((item,index)=>({id:item.id||`RUN-${String(index+1).padStart(4,'0')}`,reportId:'RPT-001',runAt:'Never',period:'Manual run',format:'CSV',rows:0,status:'ready',generatedBy:'Administrator',summary:'',...(reportRunSeedById[item.id]||{}),...item}));
  seed.reportRuns.forEach(item=>{if(!state.reportRuns.some(x=>x.id===item.id))state.reportRuns.push(JSON.parse(JSON.stringify(item)));});
  ...
  function inferAuditModule(item){
    ...
    if(text.includes('payment')||text.includes('refund')||item.icon==='▤') return 'Payments';
  ...
  syncAccessStats();
  function save(){syncAccessStats();localStorage.setItem(STORAGE_KEY,JSON.stringify(state));}
  function reset(){state=JSON.parse(JSON.stringify(seed));syncAccessStats();save();return state;}
