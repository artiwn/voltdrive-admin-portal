(function(){
  ...
  const scopedUsers=()=>state.userDirectory.filter(u=>api.companyInScope(u.companyId));
  const scopedCompanies=()=>state.companies.filter(c=>api.companyInScope(c.id));
  const scopedInvitations=()=>state.invitations.filter(i=>api.companyInScope(i.companyId));
  const scopedRequests=()=>state.accessRequests.filter(r=>api.companyInScope(r.companyId));
  ...
  function openUser(id){
    ...
    const c=company(u.companyId),r=role(u.roleId);
    openDrawer(u.name,`${u.email} · ${u.id}`,`<div class="access-profile"><div class="access-profile__head"><span class="access-profile__avatar">${esc(initials(u.name))}</span><div><h3>${esc(u.name)}</h3><p>${esc(c.name)} · ${esc(u.email)}</p><div class="access-profile__chips"><span class="ui-pill ${r.privileged?'status-review':'status-info'}">${esc(r.name)}</span><span class="ui-pill ${statusClass(u.status)}">${esc(u.status)}</span><span class="ui-pill ${u.twoFactor?'status-approved':'status-warning'}">2FA ${u.twoFactor?'enabled':'missing'}</span></div></div></div><div class="scope-callout"><strong>Effective access scope</strong><span>${esc(u.scope)}. Permissions outside this scope must be denied even when the role contains the action.</span></div><section class="ui-detail-section"><h3>Identity</h3><div class="ui-detail-grid"><div><span>Company</span><strong>${esc(c.name)}</strong></div><div><span>Role</span><strong>${esc(r.name)}</strong></div><div><span>Last login</span><strong>${esc(u.lastLogin)}</strong></div><div><span>Identity source</span><strong>${esc(u.source)}</strong></div><div><span>Created</span><strong>${esc(u.createdAt)}</strong></div><div><span>Permission count</span><strong>${permissionCount(r)}</strong></div></div></section><section class="ui-detail-section"><h3>Effective permissions</h3><div class="role-permission-groups">${userPermissionSummary(u)}</div></section></div>`,`<button class="button button--secondary" id="edit-user-access">Edit access</button><button class="button ${u.status==='suspended'?'button--primary':'button--danger'}" id="toggle-user-status">${u.status==='suspended'?'Activate user':'Suspend user'}</button>`);
    $('#edit-user-access').onclick=()=>openUserAccessForm(u.id);
    $('#toggle-user-status').onclick=()=>toggleUserStatus(u.id);
  }

  function userAccessFormHtml(u){
    return `<form id="user-access-form" class="form-grid"><label class="form-field form-field--full"><span>User</span><input value="${esc(u.name)} · ${esc(u.email)}" disabled></label><label class="form-field"><span>Company</span><select name="companyId">${scopedCompanies().map(c=>`<option value="${esc(c.id)}" ${c.id===u.companyId?'selected':''}>${esc(c.name)}</option>`).join('')}</select></label><label class="form-field"><span>Role</span><select name="roleId">${state.roles.map(r=>`<option value="${esc(r.id)}" ${r.id===u.roleId?'selected':''}>${esc(r.name)}</option>`).join('')}</select></label><label class="form-field form-field--full"><span>Access scope</span><input name="scope" value="${esc(u.scope)}" required></label><label class="form-field"><span>Account status</span><select name="status"><option value="active" ${u.status==='active'?'selected':''}>Active</option><option value="setup" ${u.status==='setup'?'selected':''}>Setup</option><option value="suspended" ${u.status==='suspended'?'selected':''}>Suspended</option></select></label><label class="form-field"><span>Two-factor authentication</span><select name="twoFactor"><option value="true" ${u.twoFactor?'selected':''}>Required / enabled</option><option value="false" ${!u.twoFactor?'selected':''}>Not enabled</option></select></label><div class="form-field form-field--full"><p class="drawer-form-actions-note">Role permissions and scope are evaluated together. A role never grants access outside the selected company/site scope.</p></div></form>`;
  }
  function openUserAccessForm(id){const u=state.userDirectory.find(x=>x.id===id);if(!u)return;openDrawer('Edit user access',`${u.name} · ${u.id}`,userAccessFormHtml(u),`<button class="button button--secondary" id="cancel-user-edit">Cancel</button><button class="button button--primary" id="save-user-edit">Save access</button>`);$('#cancel-user-edit').onclick=openUser.bind(null,id);$('#save-user-edit').onclick=()=>saveUserAccess(id);}
  function saveUserAccess(id){const u=state.userDirectory.find(x=>x.id===id),form=$('#user-access-form');if(!u||!form||!form.reportValidity())return;const data=Object.fromEntries(new FormData(form).entries());u.companyId=data.companyId;u.roleId=data.roleId;u.scope=data.scope.trim();u.status=data.status;u.twoFactor=data.twoFactor==='true';addAudit('User access changed',`${u.name} · ${role(u.roleId).name} · ${u.scope}`);api.save();toast('User access saved.');render();openUser(id);}
  function toggleUserStatus(id){const u=state.userDirectory.find(x=>x.id===id);if(!u)return;u.status=u.status==='suspended'?'active':'suspended';addAudit(u.status==='active'?'User activated':'User suspended',`${u.name} · ${u.email}`);api.save();toast(u.status==='active'?'User activated.':'User suspended.');render();openUser(id);}
  ...
  function renderRequests(){
    ...
    $('#request-list').innerHTML=requests.map(req=>{const c=company(req.companyId),r=role(req.requestedRoleId);return `<article class="request-card ${req.status!=='review'?'is-resolved':''}"><div class="request-card__main"><span class="request-card__icon">${req.status==='review'?'!':req.status==='approved'?'✓':'×'}</span><div><strong>${esc(req.name)}</strong><span>${esc(req.email)}</span><small>${esc(req.reason||'Role and scope access request.')}</small></div></div><div class="request-card__meta"><strong>${esc(req.requestedRole||r.name)}</strong><span>${esc(c.name)} · requested role</span></div><div class="request-card__meta"><strong>${esc(req.scope)}</strong><span>${esc(req.requestedAt||'Pending review')}</span></div><div class="request-card__actions">${req.status==='review'?`<button class="action-button" data-deny-request="${esc(req.id)}">Deny</button><button class="action-button action-button--primary" data-approve-request="${esc(req.id)}">Approve</button>`:`<span class="ui-pill ${statusClass(req.status)}">${esc(req.status)}</span>`}</div></article>`}).join('');
  ...
  function resolveRequest(id,approve){const req=state.accessRequests.find(r=>r.id===id);if(!req||req.status!=='review')return;req.status=approve?'approved':'denied';if(approve){let u=state.userDirectory.find(x=>x.email.toLowerCase()===req.email.toLowerCase());if(u){u.roleId=req.requestedRoleId||u.roleId;u.companyId=req.companyId||u.companyId;u.scope=req.scope;u.status='active';}else{state.userDirectory.push({id:api.nextId('USR',state.userDirectory),name:req.name,email:req.email,companyId:req.companyId||state.companies[0]?.id,roleId:req.requestedRoleId||state.roles[0]?.id,scope:req.scope,status:'active',twoFactor:false,lastLogin:'Never',source:'Access approval',createdAt:api.today()});}addAudit('Access request approved',`${req.name} · ${req.requestedRole}`);}else addAudit('Access request denied',`${req.name} · ${req.requestedRole}`);api.save();toast(approve?'Access request approved.':'Access request denied.');render();switchTab('requests');}
  ...
  function render(){state=api.getState();fillFilters();renderKpis();renderUsers();renderRoles();renderRequests();renderInvitations();}
